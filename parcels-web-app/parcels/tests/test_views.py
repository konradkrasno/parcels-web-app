...
class SearchAdvertsView(TestCase):
    def setUp(self):
        self.view = views.SearchAdvertsView()
        self.client = Client()
        ...
        self.valid_test_data = {"place": "Dębe Wielkie", "price": 200000, "area": 1000}
        self.invalid_test_data = {"price": 200000, "area": 1000}

    def test_get(self):
        response = self.client.get(
            reverse("parcels:search_adverts_form", kwargs=self.context)
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_post_if_form_is_valid(self):
        ...
        response = self.view.post(request=request, user_id=self.context["user_id"])
...
class AdvertListViewTests(TestCase):
    def setUp(self):
        self.view = views.AdvertListView()
        self.client = Client()
        self.user = User.objects.create(username="test_user", email="test@gmail.com")
        self.user.set_password("password")
        self.user.save()
    ...
    def test_get_queryset(self):
        request = RequestFactory().get(
            reverse("parcels:advert_list", kwargs=self.test_data)
        )
        self.view.setup(request)
        self.view.kwargs = self.test_data
    ...
    def test_context_data(self):
        ...
        self.assertEqual(
            context["advert_list"].values("link")[0]["link"], data[0]["link"]
        )


class AdvertDetailViewTests(TestCase):
    def setUp(self):
        self.view = views.AdvertDetailView()
        self.client = Client()
        self.user = User.objects.create(username="test_user", email="test@gmail.com")
        self.user.set_password("password")
        self.user.save()
    ...
    def test_get_queryset(self):
        request = RequestFactory().get(
            reverse("parcels:advert_detail", kwargs=self.test_data)
        )
        self.view.setup(request)
        self.view.kwargs = self.test_data
...
class FavouriteListViewTests(TestCase):
    def setUp(self):
        self.view = views.FavouriteListView()
        self.client = Client()
        self.user = User.objects.create(username="test_user", email="test@gmail.com")
        self.user.set_password("password")
        self.user.save()
    ...
    def test_get_queryset(self):
        request = RequestFactory().get(
            reverse("parcels:favourite_list", kwargs=self.test_data)
        )
        self.view.setup(request)
        self.view.kwargs = self.test_data
...
class FavouriteDetailViewTests(TestCase):
    def setUp(self):
        self.view = views.FavouriteDetailView()
        self.client = Client()
        self.user = User.objects.create(username="test_user", email="test@gmail.com")
        self.user.set_password("password")
        self.user.save()
    ...
    def test_get_queryset(self):
        request = RequestFactory().get(
            reverse("parcels:favourite_detail", kwargs=self.test_data)
        )
        self.view.setup(request)
        self.view.kwargs = self.test_data
...
class RegisterTests(TestCase):
    ...
    def test_register_if_form_is_valid(self):
        ...
        self.assertEqual(
            mail.outbox[0].subject, "Aktywacja konta w aplikacji ParcelsScraper"
        )
